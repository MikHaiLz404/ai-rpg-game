...
บรรยายฉากนี้ให้ตื่นเต้น`;
    ...
    if (NPC_TO_AGENT[npcName]) {
        ...
        if (openclawNarrative) {
            return NextResponse.json({
              ...
              source: 'openclaw',
              model: `OpenClaw Agent: ${NPC_TO_AGENT[npcName]}`,
              prompt: fullPrompt,
              usage: {
                prompt_tokens: Math.ceil(fullPrompt.length / 2), // Estimation
                completion_tokens: Math.ceil(openclawNarrative.length / 2),
                total_tokens: Math.ceil((fullPrompt.length + openclawNarrative.length) / 2)
              }
            });
        }
    }
    ...
    if (!OPENROUTER_API_KEY || OPENROUTER_API_KEY.includes('000000')) {
        ...
        if (action === 'shop_talk') {
            ...
            const talkFallbacks: Record<string, { greet: string; reply: string }> = {
                'เลโอ': { greet: `"เจ้ามาอีกแล้วหรือ? ดี — ข้าเริ่มเบื่อที่นี่พอดี"`, reply: `"ฮึ... ก็พอฟังได้ เล่าต่อเถอะ"` },
                'อารีน่า': { greet: `"ยินดีต้อนรับ ผู้มาเยือน... ลมวันนี้พัดพาเรื่องราวน่าสนใจมาให้ข้า"`, reply: `"น่าสนใจ... เจ้ามีความคิดที่ลึกซึ้งกว่าที่ข้าคาดไว้"` },
                'ดราโก้': { greet: `"ฮึ่ม... เจ้ามาถึงแล้วหรือ นั่งลงเถิด ข้ามีเรื่องจะเล่า"`, reply: `"ฮึ่ม... ข้าเคยเห็นเรื่องคล้ายกันเมื่อหมื่นปีก่อน"` },
            };
            ...
            fallback = `${playerName} ค้นพบสิ่งลึกลับ!`;
        }
        return NextResponse.json({
          narrative: fallback,
          source: 'fallback',
    ...
    return NextResponse.json(
      { narrative: "The gods are silent...", source: 'error', model: 'Error', prompt: '', usage: { total_tokens: 0 } },
      { status: 200 }
    );
  }
}
