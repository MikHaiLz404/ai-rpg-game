...
export default function Relationship() {
  ...
  return (
    <div className="p-4 bg-slate-900/95 rounded-xl shadow-2xl border border-pink-500/20 flex flex-col h-[650px] max-h-[80vh]">
      ...
      {selectedCompanion && metadata ? (
        <div className="flex-1 flex flex-col min-h-0 space-y-4 overflow-hidden">
          ...
          <div className="shrink-0 space-y-2 pb-2">
            ...
            <form onSubmit={(e) => { e.preventDefault(); if (userInput.trim() && !conversationEnded) handleTalk(selectedCompanion.id, userInput); }} className="flex gap-2 relative font-sans">
              ...
              {showGiftModal && (
                <div className="absolute bottom-full left-0 w-64 bg-slate-900 border-2 border-pink-500/30 rounded-2xl shadow-2xl p-4 z-50 animate-in fade-in slide-in-from-bottom-2 duration-300 mb-2">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-800 font-sans">{Array.from(new Set(items)).map((itemId) => { const info = ITEMS_MAP[itemId]; const isFav = selectedId ? FAVORITE_GIFTS[selectedId]?.includes(itemId) : false; return <button key={itemId} onClick={() => handleGift(itemId)} className={`w-full text-left p-2 rounded-lg text-xs md:text-sm text-slate-200 border transition-all flex justify-between items-center ${isFav ? 'bg-pink-900/30 hover:bg-pink-800/40 border-pink-500/20' : 'bg-slate-800 hover:bg-slate-700 border-white/5'}`}><span>{info ? `${info.emoji} ${info.name}` : itemId}{isFav ? ' ⭐' : ''}</span><span className="text-[8px] md:text-[10px] text-pink-500/70 font-sans">x{items.filter(i => i === itemId).length}</span></button>; })} {items.length === 0 && <div className="text-[10px] md:text-xs text-slate-600 italic text-center py-4 font-sans">ไม่มีของในกระเป๋าเลย...</div>}</div>
                </div>
              )}
